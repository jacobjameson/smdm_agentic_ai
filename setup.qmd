...
We're fully remote on Zoom, and setup problems are slow to debug for 30 people at once. Work through every part below and run the **[verification checklist](#verify-your-setup)** at the end. If something fails, **post the error in the course Slack channel** — we'll debug it there before course day rather than during class.
:::
...
If that last command printed your new repo's URL, **you're fully set up.** 🎉 Post a ✅ in the Slack channel. (You can delete the test repo afterward with `gh repo delete agentic-warmup --yes`.)
